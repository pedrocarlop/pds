...
| Public React components are agent-addressable | Every `packages/react/src/components/*.tsx` component has a matching contract under `docs/agent/components`, package README link, component index link, and preview file | `scripts/check-agent-component-contracts.mjs` and `scripts/check-react-component-previews.mjs` |
...
| Installed plugin skills can resolve the same context | Plugin context includes root adapters, canonical agent docs, foundation docs, recipes, package READMEs, and token source | `scripts/sync-plugin-context.mjs` and `pnpm plugin:context:check` |
