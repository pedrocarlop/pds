...
| Start or adopt PDS in an app | [skills/start.md](skills/start.md), target folder state or package manifest | New app recipe for empty targets; existing-app recipe for React targets | `README.md`, app root entrypoint when adoption will add styles | App setup, adoption, context refresh, and verification path are clear |
