...
| Public React APIs and supported surfaces are referenceable | Generated reference docs list current TypeScript exports and props plus component maturity, source, docs, preview, tests, a11y/focus coverage, and known gaps | `scripts/generate-react-api-reference.mjs`, `scripts/generate-supported-surface-matrix.mjs`, and `pnpm docs:reference:check` |
...
| Installed plugin skills and project-local adapters can resolve the same context | Plugin context includes root adapters plus runtime guidance for skills, components, foundations, recipes, package READMEs, reference docs, readiness evidence, evaluation scenarios, and token source; generated and adopted apps can vendor that context under `docs/pds/context` | `scripts/sync-plugin-context.mjs`, `plugins/pds/skills/start/scripts/start-pds-project.mjs`, `plugins/pds/skills/start/scripts/install-pds-project-context.mjs`, and `pnpm plugin:context:check` |
...
- `docs:lint` includes the agent-doc linter, skill-contract check,
  ...
  evaluation-scenario check, generated reference check, readiness audit, and
  plugin-context sync check.
- Component source files, preview files, and generated documentation images
  remain in one-to-one coverage.
- Generated API and supported surface reference docs remain in sync with React
  source, component docs, previews, tests, and known limitations.
...
- The PDS project guidance installer verifies that router, workflow, every
  skill workflow, readiness audit, evaluation scenarios, component contracts,
  foundations, recipes, reference docs, and package READMEs are present before
  copying context into a generated or existing app.
