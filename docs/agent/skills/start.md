...
The script creates a Vite React TypeScript app, builds and packs local
`@pds/tokens` and `pds`, installs both tarballs, imports `pds/styles.css` once,
...
Report the generated app path, that PDS was installed from local tarballs, and
the generated app commands `pnpm dev` and `pnpm build`. Do not start the
development server unless the user asks for it.
