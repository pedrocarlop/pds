...
| `iconTone` | `default`, `muted`, `accent`, `success`, `warning`, `danger`, `inactive`, `positive`, `negative` | `muted` | Tokenized icon color role. |
...
| `Item.Value.tone` | `default`, `muted`, `accent`, `success`, `warning`, `danger`, `positive`, `negative`. Use status tones only for status and performance tones only for metric direction. |
...
Item uses PDS surface color, state layers, status and performance color roles,
typography, spacing, radius, focus, disabled opacity, and motion tokens through
Cell and slot CSS. Do not add hard-coded colors, spacing, radii, transitions, or
local component-specific token names.
...
- `Item.Value.tone` uses tokenized roles; status and performance roles must not
  be mixed.
...
- Do not hard-code upstream design-system colors, remote brand assets, or row
  separators.
- Do not use status tones for financial direction or performance tones for
